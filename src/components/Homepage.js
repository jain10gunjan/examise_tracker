...
import { verbalAbility, programming, operatingsystem, computernetworks, dsa, computerarchitecture, dbms, softwareengineering, aptitudeTopics, verbalReasoning, nonVerbalReasoning } from './small-code/TopicListData'; // Adjust the path as needed

const Homepage = () => {
  const [text, setText] = useState('');
  const isTyping = true;
  const tabs = ["Aptitude", "Verbal Reasoning", "Non Verbal Reasoning", "Verbal Ability", "Programming", "OS", "CN", "DBMS", "DSA", "COA", "SE"];
  const defaultTab = "Aptitude";
  const [activeTab, setActiveTab] = useState(defaultTab);
  const [userData, setUserData] = useState({});

  const messages = useMemo(
    () => ['Percentage', 'Time and Work', 'Profit and Loss.', 'Number System', 'Profit and Loss', 'Averages', 'Boats and Streams', 'Functions'],
    []
  );
  ...
  const tabContents = {
    "Aptitude": (
      <div>
        {aptitudeTopics.map((topics, i) => {
          return (
            <TabContent topics={topics} index={i} userData={userData} />
          );
        })}
      </div>
    ),
    ...
    "Programming": (
      <div>
        {programming && programming.map((topics, i) => {
          return (
            <TabContent topics={topics} index={i} userData={userData} />
          );
        })}
        ...
        <div class="container mx-auto flex flex-col px-5 py-10 justify-center items-center">
          ...
          <div class="w-full md:w-2/3 flex flex-col mb-16">
            <div class="mt-4 mb-6 w-full p-4 border rounded-lg border-gray-200 shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700 px-12">
              ...
              <p class="mt-4 mb-2 text-base text-gray-500 sm:text-lg dark:text-gray-400">Gear up, sharpen your mind, and conquer those aptitude challenges – success awaits in your upcoming exams! 🚀✨ </p>
