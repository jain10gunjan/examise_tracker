...
const Testpage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://us-east-1.aws.data.mongodb-api.com/app/aptitude_tracker_api-fjroz/endpoint/microtestfeature?topics=${testtopics}&number=${parseInt(totalnumber)}`);
    ...
    fetchData();
  }, [apiEndpoint, testtopics, totalnumber, testid]);
  ...
  const renderAnswerStatus = (index) => {
    ...
    return (
      <>
        <p key={index} className={`text-lg mt-2 ${isCorrect ? 'text-green-500' : 'text-red-500'}`}>
          {isCorrect ? 'Correct' : 'Wrong'}
        </p>
        <p className="text-sm">You Selected: {userAnswers[index]}</p>
        <p className="text-sm mb-4 ">Correct Option: {correctAnswers[index]}</p>

        <p className="text-sm" dangerouslySetInnerHTML={{ __html: `Q${index + 1}: ${questions[index].question}` }}></p>
        <p className="text-sm mt-2" dangerouslySetInnerHTML={{ __html: `A: ${questions[index].options['A']}` }}></p>
        <p className="text-sm" dangerouslySetInnerHTML={{ __html: `B: ${questions[index].options['B']}` }}></p>
        <p className="text-sm" dangerouslySetInnerHTML={{ __html: `C: ${questions[index].options['C']}` }}></p>
        <p className="text-sm mb-2" dangerouslySetInnerHTML={{ __html: `D: ${questions[index].options['D']}` }}></p>
        <p className="text-sm" dangerouslySetInnerHTML={{ __html: `${questions[index].solution}` }}></p>
      </>
    );
  };
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < questions?.length - 1 && userAnswers[currentQuestion] !== '') {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {testtopics.split('_').map(part => <li>{part}</li>)}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
                        ...
                        <div className="flex border-gray-200">
                          <div key={questions?.[currentQuestion]?._id}>
                            ...
                            <div class="relative question-numbercontainer">
                              <p class="text-xs text-gray-600">Aptitude Questions <br />Chapter : {questions?.[currentQuestion]?.topic} </p>
                              <p class="text-xs text-gray-600">Difficulty  : {questions?.[currentQuestion]?.difficulty} </p>
                              <p class="text-xs text-gray-600">Question {currentQuestion + 1} of {questions?.length}</p>

                            </div>
                            <div class="questioncontainer">
                              <MathJax>Q{currentQuestion + 1}: {questions?.[currentQuestion]?.question.replace(/<\/?[^>]+(>|$)/g, "")}
                              </MathJax>
                            </div>
                            <div id={currentQuestion} class="flex-col space-y-2 leading-none">


                              <div className="flex">
                                <div className="w-1/2">
                                  {options.slice(0, 2).map(([optionKey, optionValue]) => (
                                    <label key={optionKey} className="block">
                                      <input
                                        ...
                                        className="mr-2 mt-4"
                                      />
                                      {`${optionKey}: ${optionValue.replace(/<\/?[^>]+(>|$)/g, "")}`}
                                    </label>
                                  ))}
                                </div>
                                <div className="w-1/2">
                                  {options.slice(2).map(([optionKey, optionValue]) => (
                                    <label key={optionKey} className="block">
                                      <input
                                        type="radio"
                                        name="answer"
